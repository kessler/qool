...
class SmartBatch {
	...
	execute(cb) {
		...
		// prefetchDequeue will call execute again when it's finished
		if (!this.dequeueBuffer && this.dequeueOpsCount > 0) {
			this.dequeueBuffer = []

			return this.prefetchDequeue()
		}
	...
	prefetchDequeue() {
		...
		stream.on('close', () => {
			debug('prefetchDequeue() stream close')

			if (error) {
				debug('prefetchDequeue error ', error)
				return this.fireCallbacks(err)
			}

			debug('prefetchDequeue() took %d', Date.now() - start)
			this.execute()
		})
	}
