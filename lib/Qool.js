...
class Qool {
	...
	enqueue(data, cb) {
		...
		this.enqueueWithKey(key, data, cb)
	}
	...
	enqueueWithKey(key, data, cb) {
		...
		this._pushToBatch({ key, value: data, userCb: cb, type: 'put' })
	}
	...
	dequeue(cb) {
		debugSilly('dequeue()')
		this._pushToBatch({ userCb: cb, type: 'del' })
	}
	...
	_loop() {
		...
		debug('_loop() executing, batch length %d', currentBatch.length)
		
		// TODO this error is also forwarded to all user callbacks from enqueue/dequeue operations
		// should we also emit an error event here perhapds?
