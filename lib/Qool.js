...
class Qool {
	...
	_leaseCallback(err, timeout, results, userCb) {
		debugSilly('_leaseCallback()')
		if (err) return userCb(err)
		if (!results) return userCb()
		if (results.length === 0) return userCb()

		let result = results[0]
		this._addLease(result.key, result.value, timeout)
		userCb(null, result.key, result.value)
	}

	leaseWithTimeout(timeout, userCb) {
		debugSilly('leaseWithTimeout()')
		this._pushToOpsQueue({ type: 'read', userCb: (err, results) => {
			this._leaseCallback(err, timeout, results, userCb)
		}})
	}
	...
	peek(userCb) {
		debugSilly('peek()')
		this.peekMany(1, (err, results) => {
			this._peekCallback(err, results, userCb)
		})
	}

	_peekCallback(err, results, userCb) {
		debugSilly('_peekCallback()')
		if (err) return userCb(err)
		if (!results) return userCb()
		if (results.length === 0) return userCb()

		userCb(null, results[0].value)
	}

	peekMany(length, userCb) {
		debugSilly('peekMany()')
		this._pushToOpsQueue({ type: 'peek', length, userCb })	
	}

	_addLease(key, value, timeout) {
		debugSilly('_addLease()')
		this._leases.add(key)
	}
